...
import static library.Controls.CellUtils.createComboBox;
 ...
 *
 * <p>By default, the ComboBoxTableCellHijack is rendered as a {@link Label} when not
 * being edited, and as a ComboBox when in editing mode. The ComboBox will, by
 * default, stretch to fill the entire table cell.
 *
 * <p>To create a ComboBoxTableCellHijack, it is necessary to provide zero or more
 ...
 * @since JavaFX 2.2
 */
public class ComboBoxTableCellHijack<S,T> extends TableCell<S,T> {
    ...
    public static <S,T> Callback<TableColumn<S,T>, TableCell<S,T>> forTableColumn(
            final StringConverter<T> converter,
            final ObservableList<T> items) {
        return list -> new ComboBoxTableCellHijack<S,T>(converter, items);
    }
    ...
    /**
     * Creates a default ComboBoxTableCellHijack with an empty items list.
     */
    public ComboBoxTableCellHijack() {
        this(FXCollections.<T>observableArrayList());
    }

    /**
     * Creates a default {@link ComboBoxTableCellHijack} instance with the given items
     ...
     */
    @SafeVarargs
    public ComboBoxTableCellHijack(T... items) {
        this(FXCollections.observableArrayList(items));
    }

    /**
     * Creates a {@link ComboBoxTableCellHijack} instance with the given items
     ...
     */
    @SafeVarargs
    public ComboBoxTableCellHijack(StringConverter<T> converter, T... items) {
     ...
     *      by the user.
     */
    public ComboBoxTableCellHijack(ObservableList<T> items) {
        this(null, items);
    }
     ...
     *      by the user.
     */
    public ComboBoxTableCellHijack(StringConverter<T> converter, ObservableList<T> items) {
    ...
    @Override public void startEdit() {
        ...
        FilteredList<DtoCaValue> filteredItems = new FilteredList(getItems(),p -> true);

        if (comboBox == null) {

            comboBox = createComboBox(this, filteredItems, converterProperty());
        ...
        Platform.runLater(() -> {
            ...
            if (selected == null || !selected.equals(editor.getText())) {
                filteredItems.setPredicate(item -> {
                    ...
                    if (item.getCaName().toUpperCase().startsWith(newValue.toUpperCase())) {
                        return true;
                    } else {
                        return false;
                    }
